...
- To maintain code running in the background even if we close terminal / VS Code 
- Help us to monitor different logs, run different scripts
- Commands
    - Open a session: tmux
    - Check the list of session: tmux ls
    - Get into (aka. re-attach) already existing session: tmux a -t <SessionID found from tmux ls>
...
conda init
### 1.4. Instal libraries / dependencies
