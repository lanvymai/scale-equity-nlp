...
import torch
import re
from transformers import AutoProcessor, BitsAndBytesConfig, AutoModelForVision2Seq, pipeline
...
def generate_prediction(pipe, question, options, image_url):
    ...
    messages = [
    {
        ...
        "content": [
            ...
            {"type": "text", "text": f"{question} Provide only the option's letter from the given choice. OPTIONS:\nA.{a}\nB.{b}\nC.{c}\nD.{d}"},  # the question
        ]
    },
    ]
    outputs = pipe(images=image, text=messages, generate_kwargs={"max_new_tokens": max_new_tokens})
...
if __name__ == "__main__":
    ...
    pipe = load_model(model_path) # Change back to args.model later
